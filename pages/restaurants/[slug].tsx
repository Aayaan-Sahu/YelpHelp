...
import { getRestaurantSlugs, getRestaurantDataFromSlug, RestaurantData } from "../../src/restaurant-api/api";
      ...
      </Head>
      {
        Object.entries(JSON.parse(data.data.data)).map(([key, value]) => {
            return (
                <div key={key}>
                <h1>{key}</h1>
                <p>{JSON.parse(data.data.data)[key]['average_rating']}</p>
                <p>{JSON.parse(data.data.data)[key]['average_ai_rating']}</p>
                </div>
            )
        })
      }
